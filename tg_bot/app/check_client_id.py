...
async def check_client_id(message: Message):
    ...
    await message.answer('Проверяю ID Клиента...')
    await run_scheduler()
    result = Storage.get_client_id(client_id)

    if result == None:
        await bot.send_message(chat_id=message.chat.id, text='Клиент не найден в системе мониторинга. Создаю учетную запись...')
        client_sys_id = create_user(client_id)
    ...
    elif result[1] == 'navixy':
        await bot.send_message(
            chat_id=message.chat.id,
            text="""
                Клиент найден в системе мониторинга My.Stavtrack!\nДля занесения объекта на сервер выберите тип устройства
            """)
        await send_page(message.chat.id, 'navixy')
        client_sys_id = result[0]
    ...
    elif (result[1] == 'stavros' or result[1] == 'stavros2'):
        await bot.send_message(
            chat_id=message.chat.id,
            text="""
                Клиент найден в системе мониторинга Online.Stavtrack!\nДля занесения объекта на сервер выберите тип устройства
            """)
        await send_page(message.chat.id, 'wialon')
        client_sys_id = result[0]
        client_platform = result[1]
        data = {
            'user_id': message.from_user.id,
            'client_sys_id': client_sys_id,
            'client_platform': client_platform
